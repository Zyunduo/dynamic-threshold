...
import sys
sys.path.append('../../')
from network.ddpg_networks import ActorNet
from network.sddpg_networks import ActorNetSpiking, ActorNetSpikingConv
...
def load_test_actor_snn_network_noise(weight_dir, bias_dir, device, batch_window=50,
                                state_num=24, action_num=2, dim=(256, 256, 256)):
    """
    Load actor snn network for testing
    :param weight_dir: directory of numpy weights
    ...
    # print(actor_net.fc1.weight)

    # print(max(actor_net.fc1.weight))

    # 8-bit LOIHI weight
    # w_max = max(actor_net.fc1.weight.max(), actor_net.fc2.weight.max(), actor_net.fc3.weight.max(), actor_net.fc4.weight.max())
    # w_min = min(actor_net.fc1.weight.min(), actor_net.fc2.weight.min(), actor_net.fc3.weight.min(), actor_net.fc4.weight.min())
    # b_max = max(actor_net.fc1.bias.max(), actor_net.fc2.bias.max(), actor_net.fc3.bias.max(), actor_net.fc4.bias.max())
    # b_min = min(actor_net.fc1.bias.min(), actor_net.fc2.bias.min(), actor_net.fc3.bias.min(), actor_net.fc4.bias.min())

    # w_s = 255/max(abs(w_max),abs(w_min),abs(b_max),abs(b_min))

    # actor_net.fc1.weight = nn.Parameter(torch.round(torch.clamp(actor_net.fc1.weight * w_s, -255, 255)))
    # actor_net.fc2.weight = nn.Parameter(torch.round(torch.clamp(actor_net.fc2.weight * w_s, -255, 255)))
    # actor_net.fc3.weight = nn.Parameter(torch.round(torch.clamp(actor_net.fc3.weight * w_s, -255, 255)))
    # actor_net.fc4.weight = nn.Parameter(torch.round(torch.clamp(actor_net.fc4.weight * w_s, -255, 255)))

    # actor_net.fc1.bias = nn.Parameter(torch.round(torch.clamp(actor_net.fc1.bias * w_s, -255, 255)))
    # actor_net.fc2.bias = nn.Parameter(torch.round(torch.clamp(actor_net.fc2.bias * w_s, -255, 255)))
    # actor_net.fc3.bias = nn.Parameter(torch.round(torch.clamp(actor_net.fc3.bias * w_s, -255, 255)))
    # actor_net.fc4.bias = nn.Parameter(torch.round(torch.clamp(actor_net.fc4.bias * w_s, -255, 255)))
    ...
    # for i in range(int(0.3 * 24 * 256)):
    #     a = random.randint(0,23)
    #     b = random.randint(0,255)
    #     while [a,b] in fc11:
    #         a = random.randint(0,23)
    #         b = random.randint(0,255)
    #     actor_net.fc1.weight[b][a] = 0.0
    #     fc11.append([a,b])
    ...
    #         a = random.randint(0,255)
    #         b = random.randint(0,255)
    #     actor_net.fc3.weight[b][a] = 0.0
    #     fc33.append([a,b])
    # for i in range(int(0.3 * 2 * 256)):
    #     a = random.randint(0,255)
    #     b = random.randint(0,1)
    #     while [a,b] in fc44:
    #         a = random.randint(0,255)
    #         b = random.randint(0,1)
    #     actor_net.fc4.weight[b][a] = 0.0
    #     fc44.append([a,b])
    ...
    # for a in range(24):
    #    for b in range(256):
    #        actor_net.fc1.weight[b][a] += random.gauss(0.0,0.05)
    
    # for a in range(256):
    #    for b in range(256):
    #        actor_net.fc2.weight[b][a] += random.gauss(0.0,0.05)
    #        actor_net.fc3.weight[b][a] += random.gauss(0.0,0.05)

    # for a in range(256):
    #    for b in range(2):
    #       actor_net.fc4.weight[b][a] += random.gauss(0.0,0.05)
...
def load_test_actor_snn_network_conv(weight_dir, bias_dir, device, batch_window=50,
                                state_num=24, action_num=2, dim=(256, 256, 256)):
    """
    Load actor snn network for testing
    :param weight_dir: directory of numpy weights
    ...
    :return: actor_net
    """
    actor_net = ActorNetSpikingConv(state_num, action_num, device,
                                batch_window=batch_window,
                                hidden1=dim[0],
                                hidden2=dim[1],
                                hidden3=dim[2])
    weights = pickle.load(open(weight_dir, 'rb'))
    bias = pickle.load(open(bias_dir, 'rb'))
    actor_net.conv1.weight = nn.Parameter(torch.from_numpy(weights[0]))
    actor_net.conv2.weight = nn.Parameter(torch.from_numpy(weights[1]))
    actor_net.fc1.weight = nn.Parameter(torch.from_numpy(weights[2]))
    actor_net.fc2.weight = nn.Parameter(torch.from_numpy(weights[3]))
    actor_net.conv1.bias = nn.Parameter(torch.from_numpy(bias[0]))
    actor_net.conv2.bias = nn.Parameter(torch.from_numpy(bias[1]))
    actor_net.fc1.bias = nn.Parameter(torch.from_numpy(bias[2]))
    actor_net.fc2.bias = nn.Parameter(torch.from_numpy(bias[3]))
    return actor_net
