...
class RandEvalGpu:
    ...
    def run_ros(self):
        ...
        fc1_u = torch.zeros(1, 256, device=device)
        fc1_v = torch.zeros(1, 256, device=device)
        fc1_s = torch.zeros(1, 256, device=device)
        fc2_u = torch.zeros(1, 256, device=device)
        fc2_v = torch.zeros(1, 256, device=device)
        fc2_s = torch.zeros(1, 256, device=device)
        fc3_u = torch.zeros(1, 256, device=device)
        fc3_v = torch.zeros(1, 256, device=device)
        fc3_s = torch.zeros(1, 256, device=device)
        fc4_u = torch.zeros(1, 2, device=device)
        fc4_v = torch.zeros(1, 2, device=device)
        fc4_s = torch.zeros(1, 2, device=device)
        # NEURON_VTH_1 = torch.tensor([0.5])
        # NEURON_VTH_2 = torch.tensor([0.5])
        # NEURON_VTH_3 = torch.tensor([0.5])
        # NEURON_VTH_4 = torch.tensor([0.5])
        PSP = [fc1_u, fc1_v, fc1_s, fc2_u, fc2_v, fc2_s, fc3_u, fc3_v, fc3_s, fc4_u, fc4_v, fc4_s]
        # fc1 = []
        # fc2 = []
        # fc3 = []
        # fc4 = []
        ...
        while not rospy.is_shutdown():
            ...
            Set new test goal
            '''
            if goal_dis < self.goal_th or is_near_obs or self.single_goal_run_ita == self.max_steps:
                ...
                elif self.single_goal_run_ita == self.max_steps:
                    failure_case += 1
                ...
                # PSP_keshihua
                # print(max(fc1[i].max() for i in range(len(fc1))))
                # print(min(fc1[i].min() for i in range(len(fc1))))
                # print(max(fc2[i].max() for i in range(len(fc2))))
                # print(min(fc2[i].min() for i in range(len(fc2))))
                # print(max(fc3[i].max() for i in range(len(fc3))))
                # print(min(fc3[i].min() for i in range(len(fc3))))
                # print(max(fc4[i].max() for i in range(len(fc4))))
                # print(min(fc4[i].min() for i in range(len(fc4))))
                # a = 0
                # b = 0
                # c = 0
                # d = 0
            ...
            action, next_PSP = self._network_2_robot_action((ddpg_state, PSP))
            move_cmd = Twist()
            # fc1.append(next_PSP[1])
            # fc2.append(next_PSP[4])
            # fc3.append(next_PSP[7])
            # fc4.append(next_PSP[10])

            if self.single_goal_run_ita < 20:
    ...
    def _network_2_robot_action(self, state):
        ...
        with torch.no_grad():
            if self.is_spike:
                ...
                action, next_PSP = self.actor_net((state, PSP), 1)
    ...
    def _near_obstacle(self, pos):
        ...
        for poly in self.obstacle_poly_list:
            ...
            if min(self.robot_scan) < 0.18:
                if self.single_goal_run_ita < 50:
                    done = False
                else:
                    done = True
                break
    ...
    def _robot_scan_cb(self, msg):
        ...
        # 0.2 / 6.0
        # self.robot_scan[2]=0.2
        # self.robot_scan[8]=0.2
        # self.robot_scan[14]=0.2

        # GN
        # self.robot_scan_ = copy.deepcopy(self.robot_scan)
        # for i in range(18):
        #     self.robot_scan[i] = np.clip(self.robot_scan[i] + random.gauss(0.0,1), 0.2, 6.0)
